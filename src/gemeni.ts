...
function calculateAreaPercent(d: Detection): number {
...
async function analyzeWithVertexAI(detections: Detection[]): Promise<any> {
...
Respond in JSON format:
{
  ...
  const formatted = detections
    .map(
      (d, i) =>
        `Detection ${i + 1}: ${d.name} (${
          d.probability
        }%) covering ${calculateAreaPercent(d).toFixed(1)}% of screen`
    )
    .join("\n");
